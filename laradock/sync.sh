...
# the installation and usage of laradock with laradock-sync.
...
# USAGE EXAMPLE:
# Install laradock-sync: ./sync.sh install
...
display_options () {
    ...
    print_style "   install" "info"; printf "\t\t Installs laradock-sync gem on the host machine.\n"
    print_style "   up [services]" "success"; printf "\t Starts laradock-sync and runs laradock compose.\n"
    print_style "   down" "success"; printf "\t\t\t Stops containers and laradock-sync.\n"
    ...
    print_style "   clean" "danger"; printf "\t\t Removes all files from laradock-sync.\n"
}
...
if [ "$1" == "up" ] ; then
    ...
    print_style "Installing laradock-sync\n" "info"
    gem install docker-sync

elif [ "$1" == "sync" ]; then
    print_style "Manually triggering sync between host and laradock-sync container.\n" "info"
    docker-sync sync;

elif [ "$1" == "clean" ]; then
    print_style "Removing and cleaning up files from the laradock-sync container.\n" "warning"
    docker-sync clean
else
