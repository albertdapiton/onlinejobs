...
# Publish any versions of the laradock image not yet pushed to jenkinsci/jenkins
# Arguments:
...
# Try tagging with and without -f to support all versions of laradock
docker-tag() {
...
get-manifest() {
    local tag=$1
    curl -q -fsSL -H "Accept: application/vnd.laradock.distribution.manifest.v2+json" -H "Authorization: Bearer $TOKEN" "https://index.laradock.io/v2/jenkinsci/jenkins/manifests/$tag"
}
