...
# check dependencies
(
    type docker &>/dev/null || ( echo "laradock is not available"; exit 1 )
...
function get_jenkins_url {
    ...
    echo "http://$DOCKER_IP:$(laradock port "$SUT_CONTAINER" 8080 | cut -d: -f2)"
}
...
function unzip_manifest {
    local plugin=$1
    local work=$2
    bash -c "laradock run --rm -v $work:/var/jenkins_home --entrypoint unzip $SUT_IMAGE -p /var/jenkins_home/plugins/$plugin META-INF/MANIFEST.MF | tr -d '\r'"
}
